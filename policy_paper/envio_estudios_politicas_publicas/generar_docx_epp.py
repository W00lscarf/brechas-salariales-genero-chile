# -*- coding: utf-8 -*-
"""Convierte manuscrito_epp.md en el .docx de envío para la Revista Estudios
de Políticas Públicas (12 pt, espaciado simple, cursivas, tablas y figuras
integradas con fuente).
...
MD = os.path.join(AQUI, 'manuscrito_epp.md')
OUT = os.path.join(AQUI, 'manuscrito_epp.docx')
DIR_PAPER = os.path.dirname(AQUI)  # policy_paper/
...
while i < n:
    ...
    if linea.startswith('## '):
        h = linea[3:].strip()
        en_refs = h.startswith('Referencias bibliográficas')
        encabezado(h, 1)
        i += 1
        continue
    if linea.startswith('### '):
    ...
    m = re.match(r'^\*\*(Tabla|Figura) (\d+)\*\*$', linea.strip())
    if m:
        tipo = m.group(1)
        ...
        while i < n and not lineas[i].strip():
            i += 1
        # Nota / Fuente
        if i < n and lineas[i].strip().startswith(('Nota:', 'Fuente:')):
    ...
    if linea.startswith('- '):
        ...
        while i < n and lineas[i].strip() and not lineas[i].startswith(('- ', '#', '|', '>')) \
                ...
                and not re.match(r'^\*\*(Tabla|Figura) \d+\*\*$', lineas[i].strip()):
            txt += ' ' + lineas[i].strip()
            i += 1
