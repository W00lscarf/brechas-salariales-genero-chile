# Misma ocupación, distinto salario: la brecha de género que la composición no explica en Chile
...
Este artículo estima la brecha salarial de género en Chile utilizando el conjunto más granular de controles observables disponible en los microdatos públicos: la ocupación a cuatro dígitos CIUO-08 (354 categorías), además de horas trabajadas, educación y tipo de institución, edad, geografía, formalidad, sector y estructura familiar (CASEN 2022 y 2024, *n* = 174.924; ESI 2018-2024). La brecha ajustada se sitúa entre -11% y -16% en catorce especificaciones y no disminuye al agregar controles. La asociación de la parentalidad con el ingreso es asimétrica: tener hijos se asocia a un premio de +5.0% entre los hombres y a una penalización adicional de -7.8% entre las mujeres, dentro de la misma ocupación. Ninguna de las 227 ocupaciones examinadas presenta una brecha favorable a las mujeres que sobreviva la corrección por comparaciones múltiples; 66 presentan brechas significativas en su contra. La brecha se concentra en los hogares de menor nivel socioeconómico y exhibe forma de U a lo largo de la distribución salarial. Una descomposición por emparejamiento con soporte común replica el resultado central. Se concluye que el diferencial corresponde principalmente a retornos desiguales en el sentido de Oaxaca-Blinder y se derivan seis recomendaciones de política.
...
This article estimates the gender wage gap in Chile using the most granular set of observable controls available in public microdata: 4-digit ISCO-08 occupation (354 categories), together with hours worked, education and type of institution, age, geography, formality, sector, and family structure (CASEN 2022 and 2024, *n* = 174,924; ESI 2018-2024). The adjusted gap ranges between -11% and -16% across fourteen specifications and does not shrink as controls are added. The association between parenthood and income is asymmetric: having children is associated with a +5.0% premium among men and an additional -7.8% penalty among women, within the same occupation. None of the 227 occupations examined shows a gap favoring women that survives correction for multiple comparisons; 66 show significant gaps against them. The gap is concentrated in lower socioeconomic status households and is U-shaped along the wage distribution. A matching decomposition with common support replicates the central result. We conclude that the differential corresponds mainly to unequal returns in the Oaxaca-Blinder sense, and six policy recommendations are derived.
...
> **¿Persiste la brecha salarial de género en Chile una vez descontado el conjunto más granular de controles composicionales observables que permiten los datos públicos —ocupación exacta, horas trabajadas, educación y su calidad institucional, geografía, régimen de formalidad, sector y estructura familiar—? Y, de persistir, ¿en qué segmentos de la población es mayor?**
...
Nota: ESI = Encuesta Suplementaria de Ingresos; CASEN = Encuesta de Caracterización Socioeconómica Nacional. Fuente: elaboración propia.
...
6. **Descomposición por emparejamiento exacto con soporte común** (Ñopo, 2008), con bootstrap de 500 réplicas para la inferencia (sección 7.1).
...
Nota: Estimaciones de mínimos cuadrados ponderados con factores de expansión y errores estándar robustos por conglomerado × año. IC = intervalo de confianza; pp = puntos porcentuales. Fuente: elaboración propia a partir de CASEN 2022 y 2024.
...
Nota: Referencia: coeficientes masculinos. Los porcentajes indican la proporción de la brecha total atribuible a la composición de cada factor; los valores negativos corresponden a factores que operan a favor de las mujeres. Fuente: elaboración propia a partir de CASEN 2022 y 2024.
...
Nota: Modelo de mínimos cuadrados ponderados con interacciones; categoría de referencia del estado civil: casada o conviviente. Fuente: elaboración propia a partir de CASEN 2022 y 2024.
...
Nota: Cada punto representa una ocupación CIUO-08 a 4 dígitos. En rojo, ocupaciones con brecha ajustada estadísticamente significativa (p < .05); en gris, no significativas. Fuente: elaboración propia a partir de CASEN 2022 y 2024.
...
Nota: Síntesis de las estimaciones de las secciones 4.1 a 4.5. Fuente: elaboración propia.
...
| *...familiares: priorizan los hijos sobre la carrera* | Interacción mujer×hijos, mujer×estado civil | **Tener hijos se asocia a +5.0% entre los hombres y a un -7.8% adicional entre las mujeres.** Una «elección familiar» simétrica no produce asociaciones asimétricas por sexo |
...
Nota: Los valores provienen de las estimaciones de las secciones 4 y 7. Fuente: elaboración propia.
...
Nota: Regresión cuantílica (cuantiles 10 a 90) con el conjunto completo de controles; sin factores de expansión por restricción del método. La línea discontinua indica la brecha media estimada por mínimos cuadrados ponderados. Fuente: elaboración propia a partir de CASEN 2022 y 2024.
...
Los resultados centrales fueron sometidos a la batería de robustez que un proceso de revisión exigiría . La Tabla 8 presenta la brecha ajustada con ocupación granular bajo cada especificación; la Figura 3 la representa gráficamente.
...
Nota: Salvo indicación en contrario, todas las especificaciones incluyen control por ocupación a 4 dígitos, factores de expansión y errores estándar robustos por conglomerado × año. Fuente: elaboración propia a partir de CASEN 2022 y 2024.
...
Nota: Descomposición ponderada por factores de expansión; Δ0 compara hombres y mujeres dentro del soporte común, reponderando la distribución masculina de celdas a la distribución femenina. Intervalo de confianza al 95% por bootstrap (500 réplicas) para Δ0 con ocupación a 4 dígitos: [15.3, 19.0]. La equivalencia se calcula como −Δ0/(1+Δ0). Fuente: elaboración propia a partir de CASEN 2022 y 2024.
...
Nota: Barras oscuras: especificaciones con ocupación a 4 dígitos. Fuente: elaboración propia a partir de CASEN 2022 y 2024.
...
Arrow, K. (1973), "The Theory of Discrimination". En Ashenfelter, O. y Rees, A., eds., *Discrimination in Labor Markets*. Princeton: Princeton University Press.
...
Bertrand, M. (2011), "New Perspectives on Gender". En Ashenfelter, O. y Card, D., eds., *Handbook of Labor Economics*, vol. 4B. Ámsterdam: Elsevier.
...
Blau, F. y Kahn, L. (2017), "The Gender Wage Gap: Extent, Trends, and Explanations", *Journal of Economic Literature*, 55(3): 789-865.
...
Croson, R. y Gneezy, U. (2009), "Gender Differences in Preferences", *Journal of Economic Literature*, 47(2): 448-474.
...
Instituto Nacional de Estadísticas (2018-2024), *Encuesta Suplementaria de Ingresos*, microdatos. Santiago: INE.

Kleven, H., Landais, C. y Leite-Mariante, G. (2025), "The Child Penalty Atlas", *Review of Economic Studies*.

Kleven, H., Landais, C. y Søgaard, J. E. (2019), "Children and Gender Inequality: Evidence from Denmark", *American Economic Journal: Applied Economics*, 11(4): 181-209.
...
Ley N.º 20.348 (2009), Resguarda el derecho a la igualdad en las remuneraciones. Santiago: Diario Oficial de la República de Chile.

Ley N.º 20.545 (2011), Modifica las normas sobre protección a la maternidad e incorpora el permiso postnatal parental. Santiago: Diario Oficial de la República de Chile.
...
Ministerio de Desarrollo Social y Familia (2022, 2024), *Encuesta de Caracterización Socioeconómica Nacional (CASEN)*, microdatos. Santiago: Observatorio Social.

Mueller, G. y Plug, E. (2006), "Estimating the Effect of Personality on Male and Female Earnings", *ILR Review*, 60(1): 3-22.
...
Niederle, M. y Vesterlund, L. (2007), "Do Women Shy Away from Competition? Do Men Compete Too Much?", *Quarterly Journal of Economics*, 122(3): 1067-1101.

Ñopo, H. (2006), *The Gender Wage Gap in Chile 1992-2003 from a Matching Comparisons Perspective*. Washington D.C.: Banco Interamericano de Desarrollo.
...
Organización Internacional del Trabajo (1951), *Convenio sobre igualdad de remuneración, 1951 (núm. 100)*. Ginebra: OIT.

Organización Internacional del Trabajo (2018), *Informe mundial sobre salarios 2018/19: ¿Qué hay detrás de la brecha salarial de género?* Ginebra: Oficina Internacional del Trabajo.
...
Petersen, T. y Morgan, L. (1995), "Separate and Unequal: Occupation-Establishment Sex Segregation and the Gender Wage Gap", *American Journal of Sociology*, 101(2): 329-365.
