# -*- coding: utf-8 -*-
"""Convierte README_EN.md en un PDF de working paper listo para envio.
...
MD = os.path.join(BASE, 'README_EN.md')
OUT = os.path.join(BASE, 'same_occupation_different_pay_wp.pdf')
...
subtitle = next(l[3:].strip() for l in head if l.startswith('## '))

story.append(Spacer(1, 1.15 * inch))
story.append(Paragraph(fmt(title), S['title']))
story.append(Paragraph(fmt(subtitle), S['subtitle']))
story.append(Spacer(1, 0.35 * inch))
story.append(Paragraph('Working Paper', S['meta']))
...
while i < n:
    ...
    if line.startswith('## '):
        h = line[3:].strip()
        in_refs = h.startswith('10. References')
        in_abstract = h == 'Abstract'
        if h.startswith(('1. Introduction', '10. References')):
